...
<nav class="w-1/5 bg-emerald-700 overflow-y-scroll">
  ...
  <div class="w-full flex-1 shadow-xl rounded-3xl text-gray-300">
    <div class=" text-center pt-5 font-serif font-bold text-3xl ">Data Gudang</div>
  ...
  <tbody>
    ...
    $data = mysqli_query($connection, "SELECT * FROM gudang
    INNER JOIN barang ON gudang.no_barang = barang.no_barang INNER JOIN faktur ON gudang.no_faktur = faktur.no_faktur");
    $no=1; 
    foreach ($data as $item): 
    ?>
    <tr>
      <th scope="col ">
        <div class="ml-2 m-3"> <?= $no++; ?>
        </div></th>
        <th scope="col">
        <div class=" w-full "> <?= $item['nama_gudang']  ?>
        </div></th>
        <th scope="col">
